...
@implementation ZYHTfmdbHomeScrollADHelper
...
- (NSString *)getHomeScrollAD_Path{
    ...
    dbPath = [dbPath stringByAppendingPathComponent:@"/zyht_homeScrollADInfo.sqlite"];
    NSLog(@"docPath = %@",dbPath);
    
    return dbPath;
}

static NSString * const isCreateTable_HomeRecommendDBFlag = @"isCreateTable_HomeScrollADDBFlag";
...
- (BOOL)createTable_HomeScrollAD{
    ...
    NSString *dbPath = [self getHomeScrollAD_Path];
    ...
    if ([fileManager fileExistsAtPath:dbPath] == NO) {
        ...
        if ([database open]) {
            ...
            BOOL result = [database executeUpdate:@"create table zyht_homeScrollADInfo (id integer PRIMARY KEY AUTOINCREMENT NOT NULL,img_url text NOT NULL,imgData blob NOT NULL);"];
            [database close];
