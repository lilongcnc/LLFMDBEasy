...
@implementation ZYHTfmdbHomeRecommendInfoHelper
ILSingleton_M
...
- (NSString *)getHomeRecommendDB_Path{
    ...
    dbPath = [dbPath stringByAppendingPathComponent:@"/zyht_homeRecommendInfo.sqlite"];
    NSLog(@"docPath = %@",dbPath);
    
    return dbPath;
}

static NSString * const isCreateTable_HomeRecommendDBFlag = @"isCreateTable_HomeRecommendDBFlag";
- (BOOL)createTable_HomeRecommendDB{
    ...
    NSString *dbPath = [self getHomeRecommendDB_Path];
    ...
    if ([fileManager fileExistsAtPath:dbPath] == NO) {
        ...
        if ([database open]) {
            ...
            BOOL result = [database executeUpdate:@"create table zyht_homeRecommendInfo (id integer PRIMARY KEY AUTOINCREMENT NOT NULL,name text NOT NULL,age integer NOT NULL);"];
            [database close];
